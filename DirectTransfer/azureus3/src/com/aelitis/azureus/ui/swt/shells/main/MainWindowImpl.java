	...
	AEDiagnosticsEvidenceGenerator, MdiEntryLogIdListener, UIUpdatable
{
	 ...
	 */
	private void createWindow(IUIIntializer uiInitializer) throws UnknownHostException, SocketException {
		...
		try {
			...
			TrackerChanged tc = new TrackerChanged();
			
			long trackerChangedNotified = COConfigurationManager.getLongParameter("TrackeraChangedNotified");
			
			if ( ( tc.trackerChanged() ) && (trackerChangedNotified == 0) ) {
				
				new TrackerChangedDialog(shell,SWT.NONE).open();
				COConfigurationManager.setParameter("TrackeraChangedNotified", new Long(1));
			}
			...
			if(ConfigurationChecker.isNewVersion()){
				...
				new WelcomeWindow(this.getShell());
			}
		...
		text.addListener(SWT.KeyDown, new Listener() {
			...
			public void handleEvent(Event event) {
				...
				if (event.character == SWT.CR) {
					...
					new SearchDialog(shell,SWT.NONE).open();
				}
			}
		});
