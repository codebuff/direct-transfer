	...
	implements ShareManager, TOTorrentProgressListener, ParameterListener, AEDiagnosticsEvidenceGenerator
{
	...
	private HostIp hostip = new HostIp();
	...
	getAnnounceURLs()
		...
		throws ShareException, UnknownHostException, SocketException
	{
		if ( announce_urls == null ){
			...
			if ( protocol.equalsIgnoreCase( "DHT" )){
				...
				if( hostip.isConnectedToNU()){
					COConfigurationManager.setParameter("Tracker IP", hostip.getHost_ip());
					COConfigurationManager.setParameter("TrackeraChangedNotified", new Long(0));
