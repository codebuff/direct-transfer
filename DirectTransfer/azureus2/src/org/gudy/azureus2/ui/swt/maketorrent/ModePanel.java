...
public class ModePanel extends AbstractWizardPanel<NewTorrentWizard> {
  ...
  public void show() {
   ...
   /* final Button btnLocalTracker = new Button(panel, SWT.RADIO);   
    ...
    //COConfigurationManager.setParameter("Tracker IP", "172.19.4.46");//implemented for testing
  ...
  /*  btnDHTTracker.addListener(SWT.Selection, new Listener() {
    ...
    Label infTitle = new Label(panel,SWT.NULL);
    infTitle.setAlignment(SWT.CENTER);
    infTitle.setText("\nPlease Note ");
    
    
    Label infText = new Label(panel,SWT.NULL);
    infText.setText(" I am still waiting for a reply from TCO until then please don't use this \n functionality \n\t\t\t----------\n once properly configured I "
    		+ "will provide you a link and then you can replace \n 'http:// type here' in Announce URL box with that link\n\t\t\t----------\n\t\t\t\t\t       Deepankar Tyagi");
