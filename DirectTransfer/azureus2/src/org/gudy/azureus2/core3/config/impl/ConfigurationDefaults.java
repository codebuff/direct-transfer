...
public class ConfigurationDefaults {
  ...
  ConfigurationDefaults() 
  {
    ...
    //introduced in first nu version
    def.put("TrackeraChangedNotified",FALSE);
    ...
    def.put("TCP.Listen.Port", new Long( 6885 ));
    def.put("TCP.Listen.Port.Enable", TRUE );
    def.put("TCP.Listen.Port.Override", "");
    def.put("UDP.Listen.Port", new Long( 6885 ));
    def.put("UDP.Listen.Port.Enable", TRUE );
    def.put("UDP.NonData.Listen.Port", new Long( 6885 ));	// two effective enablers for this, dht + tracker udp client
    ...
    def.put("User Mode", ONE);//earlier zero
	...
	if ( Constants.isAndroid ){
		...
		f = new File(docPath, "Azureus Downloads");
			...
			// switch to direct transfer for new installs
		if ( !f.exists()){
			f = new File(docPath, "Direct Transfers");
		}
	}
	
	def.put("Default save path", f.getAbsolutePath());
    
    def.put("update.start",FALSE);//earlier true
    def.put("update.periodic",FALSE);//earlier true
    def.put("update.opendialog",FALSE);//earlier true
    def.put("update.autodownload", FALSE);

    def.put("Send Version Info", FALSE);//earlier true
    ...
    def.put( "Low Port", new Long(6885) );
    ...
    def.put( "Tracker Public Enable", TRUE );//earlier false
    def.put( "Tracker Log Enable", FALSE );
    def.put( "Tracker Port Enable", TRUE );//earlier false
   ...
   // def_put( "Tracker IP", "172.19.5.123", new IPVerifier());//didn't work
    //def.put("Tracker IP", "172.19.5.1"); didn't work
    ...
    def.put( "Peer Source Selection Default.DHT", TRUE);//might be changed 
    ...
    def.put( "Sharing Protocol", "http" );//earlier "DHT"
    ...
    def.put( "Sharing Permit DHT", FALSE);//earlier true
    ...
    def.put( "Auto Update", TRUE );//might be changed 
    ...
    def.put( "subscriptions.auto.start.downloads", FALSE );//earlier true
    def.put( "subscriptions.auto.start.min.mb", ZERO );
    def.put( "subscriptions.auto.start.max.mb", ZERO );
    
    def.put( "Show Side Bar", FALSE );//earlier true
